...
describe("auro-flightline", () => {
  ...
  it("shows canceled summary when contains canceled segment", async () => {
    ...
    expect(
      summarySegment.hasAttribute("canceled"),
      "Canceled attribute not set",
    ).to.be.true;
  });
  ...
  it("when flightline is canceled, all segments are canceled", async () => {
    ...
    expect(segments[0].hasAttribute("canceled"), "Canceled attribute not set")
      .to.be.true;
    expect(
      segments[0].hasAttribute("destinationCanceled"),
      "destinationCanceled attribute not set",
    ).to.be.true;
    expect(segments[1].hasAttribute("canceled"), "Canceled attribute not set")
      .to.be.true;
    expect(
      segments[1].hasAttribute("destinationCanceled"),
      "destinationCanceled attribute not set",
    ).to.be.true;
  });
  ...
  it("when all segments are canceled, summary segment is fully canceled", async () => {
    ...
    expect(
      summarySegment.hasAttribute("partialCancel"),
      "partialCancel attribute not set",
    ).to.be.true;
    expect(
      summarySegment.hasAttribute("destinationCanceled"),
      "destinationCanceled attribute not set",
    ).to.be.true;
  });
  ...
  it("when only a middle segment is canceled, summary segment is partially canceled", async () => {
    ...
    const summarySegment = el.shadowRoot.querySelector("auro-flight-segment");
    expect(summarySegment.hasAttribute("canceled"), "Canceled attribute is set")
      .to.be.false;
    expect(
      summarySegment.hasAttribute("partialCancel"),
      "partialCancel attribute not set",
    ...
    expect(
      summarySegment.hasAttribute("destinationCanceled"),
      "destinationCanceled attribute is set",
    ).to.be.false;
  });
...
describe("auro-flight-segment", () => {
  it("with a layover", async () => {
    ...
    await expect(el.shadowRoot.querySelectorAll(".duration").length).to.equal(
      1,
    );
  });
  ...
  it("with a layover no duration", async () => {
    ...
    await expect(el.shadowRoot.querySelector("span").innerText).to.equal(
      "2 Stops",
    );
    await expect(el.shadowRoot.querySelectorAll(".duration").length).to.equal(
      0,

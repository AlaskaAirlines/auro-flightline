...
/**
 * Replace all instances of @aurodesignsystem, flightline, Flightline, auro and Auro accordingly
 */
...
function formatTemplateFileContents(content, destination) {
  ...
  result = result.replace(/\flightline(?!\()/g, nameExtractionData.name);
  result = result.replace(/\Flightline(?!\()/g, nameExtractionData.nameCap);
